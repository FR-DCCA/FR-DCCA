...
class Wrapper:
        ...
        for epoch in range(self.epoch_num):
            if early_stop == False:
                ...
                if epoch_val_loss < min_val_loss:
                    ...
                    best_model = copy.deepcopy(self.model.state_dict())
                    print('Min loss %0.4f' % min_val_loss)
                    epochs_no_improve = 0
                else:
                    epochs_no_improve += 1
    ...
    def train_epoch(self, train_dataloader: torch.utils.data.DataLoader):
        ...
        for batch_idx, (x, y, label) in enumerate(train_dataloader):
            ...
            repre = torch.cat([view1_specific, view1_shared], dim=1)
            ...
            self.optimizer.step()
            self.clf_.fit(repre.cpu().data.numpy(), label.cpu().data.numpy())
    ...
    def val_epoch(self, val_dataloader: torch.utils.data.DataLoader):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (x, y, val_label) in enumerate(val_dataloader):
                ...
                # cal f1 and acc
                pred = self.clf_.predict(repre.cpu().data.numpy())
    ...
    def test_by_svm(self, train_dataloader: torch.utils.data.DataLoader, test_dataloader: torch.utils.data.DataLoader):
        ...
        train_view1 = np.empty(shape=(0, self.latent_dims*2))  # use only view1 model result to classify
